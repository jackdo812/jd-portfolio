...
function FullWidthTabs ({dataACF}) {
    ...
    return (
      ...
      listProjectsDetailsLength > 1 ? (
        <Tabs className={`text-forest mt-4 shadow-[0_0_4px_1px_rgba(128,128,128,1)] rounded-[10px]`}>
          ...
          <TabList className={`text-foggy font-bold rounded-t-[10px] bg-leaf flex text-center ${listProjectsDetailsLength === 3 && 'text-[0.8rem] min-[400px]:text-[1rem] md:text-[1.1rem]'}`}> 
            {listProjectsDetails.map((item, index) => {
                return (
                    <Tab className={`${listProjectsDetailsLength === 2 ? 'w-[50%] text-[1.1rem]' : 'w-[33.3%]'} cursor-pointer p-2`} selectedClassName={`${listProjectsDetailsLength === 2 ? 'w-[50%]' : 'w-[33.4%]'} bg-foggy text-forest font-bold underline rounded-t-[7px] decoration-soil decoration-4 underline-offset-4`} key={index}>{item.single_project_details_heading}</Tab>
                );
            })}
          </TabList>
        ...
        listProjectsDetails.map((item, index) => {
            return (
              <div key={index} className='bg-foggy rounded mt-4 shadow-[0_0_4px_1px_gray] py-2'>
                <Fade>
